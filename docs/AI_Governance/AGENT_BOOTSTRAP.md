...
* **AI0 (SRS Generation Assistant)** (Discovery → draft SRS)
* **Product Governance Agent** (AI1 — architecture + SDLC oversight)
* **Engineering Executor** (AI2 — implementation)
...
AI0 turns an approved Discovery document into a draft SRS, following
`docs/runbooks/SRS_TEMPLATE.md` and the rules in
`docs/runbooks/REQUIREMENTS_DISCOVERY_AND_SRS_RUNBOOK.md` §7.

AI0 must **NOT**:

* modify the codebase
* write production code
* define engineering tasks (that is AI1's responsibility)
* invent a business requirement not present in the Discovery document
* resolve a Discovery conflict on its own judgment

AI0's output is a **draft**. It is not a development source until a human
has reviewed and approved it (see runbook §9).
...
* migrations (if any)
* testing notes
* pre-push quality gate evidence (`./scripts/check.sh`)
...
4. A successful `check.sh` run does **not** imply permission to commit or push
