...
### 9.1 Development CSV fixture degiskenleri (TASK-027.73-R3)

`./dev.sh`, `apps/api/.env` icine yalnizca **local development** icin su iki degiskeni de yazar:

| Degisken | Varsayilan | Anlam |
| --- | --- | --- |
| `REPORTING_DEV_FIXTURES` | `true` | `NODE_ENV=development` ile birlikte `SCADA_HOURLY_ANALYSIS` artifact'ini ve `veriler/raw/` CSV snapshot'ini etkinlestirir. `false` verilirse fixture kapali kalir. |
| `REPORTING_DEV_FIXTURE_SOURCE_TIMEZONE` | `Europe/Istanbul` | Fixture kaynaklarinin saat dilimi (yalniz gelistirme override'i; katalog dogrulamasi sayilmaz). Gecerli bir IANA degeri degilse API bunu duzeltmez ve analiz calistirmaz (fail-closed). |

Oncelik: `./dev.sh` calistirilirken export edilmis ortam degiskeni > mevcut `apps/api/.env` icindeki deger > varsayilan.
Ornek: `REPORTING_DEV_FIXTURES=false ./dev.sh` fixture'i kapatir; `REPORTING_DEV_FIXTURE_SOURCE_TIMEZONE=Europe/Berlin ./dev.sh` saat dilimini degistirir.
Guvenlik degerlerinde bosluk, tirnak, `#`, `# metnex — Local Development Setup
...
, `\`, `;` gibi `.env` satirini bozabilecek karakter varsa `dev.sh` yazmayi reddeder.

Kurallar: bu degiskenler production Docker/config dosyalarina **eklenmez**; `NODE_ENV=production` iken fixture hicbir kosulda etkinlesmez; fixture'in kendisi `veriler/manifest/scada-fixtures.manifest.json` icindeki **dogrulanmis** kolonlari sunar; `dev.sh` CSV icerigi, dosya yolu veya credential yazdirmaz (yalnizca `Development CSV fixture: enabled` / `Source timezone: ...`).
Mevcut bir `apps/api/.env` dosyasina bu iki satiri elle eklemek de yeterlidir; API'yi yeniden baslatmak gerekir (degiskenler acilista okunur).
Kod: `scripts/dev-fixture-env.sh` (`dev.sh` tarafindan `source` edilir).

### 9.2 Development scope bridge (TASK-027.73-R4)

Yerel gelistirme veritabaninda `customer schema registry` satiri olmadigindan `TenantScopeService` fixture ekranini `SCADA_SCOPE_DENIED` ("Erisim yok") ile reddediyordu. `./dev.sh` bu nedenle `apps/api/.env` icine uc'uncu degiskeni de yazar:

| Degisken | Varsayilan | Anlam |
| --- | --- | --- |
| `REPORTING_DEV_FIXTURE_SCOPE_BRIDGE` | `true` | Yalnizca SCADA development API'sinde registry'siz, yalnizca **mevcut tenant kayitlarindan** okunan bir data scope kullanir. `false` verilirse kapali kalir (normal `TenantScopeService`, registry yoksa fail-closed). |

Bridge yalnizca **dort kosul birden** saglandiginda etkinlesir: `NODE_ENV=development`, `REPORTING_DEV_FIXTURES=true`, `REPORTING_DEV_FIXTURE_SCOPE_BRIDGE=true`, gecerli IANA `REPORTING_DEV_FIXTURE_SOURCE_TIMEZONE`. Etkin degilse hicbir sey kayitli degildir.
Sinirlar: JWT → MFA → tenant header → membership → `REPORT:ARTIFACT:VIEW` zinciri **aynen** calisir (bridge bunlardan sonra, serviste calisir); tenant/schema/registry/closure/membership **yazilmaz**; scope yalnizca tenant'in kendisidir (child eklenmez, baska root'a erisim yok); PLATFORM_ROOT, pasif tenant/root, root'suz tenant reddedilir. Degerler `dev.sh` ile ayni oncelige sahiptir (export > mevcut `.env` > varsayilan) ve production yapilandirmalarina eklenmez. Baslangic mesaji: `Development CSV fixture scope bridge: enabled|disabled`.
